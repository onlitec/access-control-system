...
| Fase | Escopo | Status |
|---|---|---|
| **0** | Equipamento(s) na rede: teste bruto de conectividade (ping, `deviceInfo` via ISAPI) para terminal e/ou controladora, leitura da documentação ISAPI do fabricante (a obter, inclusive datasheet da DS-K2812), confirmação/ajuste dos endpoints e da topologia assumidos neste documento (item 7.6 em especial — decisão local vs. central). | Pendente — depende do equipamento físico estar na rede |
...
| **3** | Cadastro de face de morador (`enrollFace`) — ligado ao formulário de moradores, como hoje acontece com `txSerial`/`cardSerial` para o Guarita — e sincronização do nível de acesso (`syncAccessLevel`) para o equipamento. | Pendente — depende de endpoints ISAPI confirmados |
| **4** | Recepção de eventos de reconhecimento em tempo real → `AccessEvent` via `emitEvent`, incluindo eventos de alarme (violação/tamper/coação). | Pendente — depende de endpoints ISAPI confirmados |
...
| **6** | Importação de histórico de eventos do equipamento, no mesmo espírito de `NiceGuaritaService.startImportStoredEvents` (import job em segundo plano, idempotente). | Pendente |

### ⏸️ Decisão em aberto: Fases 3/4/6 sem o equipamento físico (retomar daqui)

Pergunta levantada em 2026-07-07: dá pra implementar as Fases 3, 4 e 6 mesmo sem o DS-K1T673/DS-K2812 na rede?

**Resposta**: parcialmente sim. O ISAPI da Hikvision para controle de acesso segue uma convenção bastante conhecida publicamente no ecossistema (não é documentação exclusiva do fabricante) — por exemplo, endpoints como `/ISAPI/Intelligent/FDLib/FDSetUp` (upload de foto facial) e `/ISAPI/Event/notification/alertStream` (long-poll de eventos) são padrões recorrentes em integrações de terceiros com equipamentos Hikvision. Dá pra escrever uma implementação "melhor esforço" dessas fases com base nessa convenção.

**O trade-off que ficou pendente de decisão**: esse código sairia **não verificado** — a estrutura geral tende a estar certa, mas detalhes finos (nome exato de campo, XML vs JSON, formato do multipart da foto) só seriam confirmados quando o equipamento real estivesse na rede, e nesse momento a depuração seria feita sem mim observando os logs ao vivo.

**Status: decisão NÃO tomada ainda** — o usuário pediu para salvar este ponto e continuar depois. Ao retomar, perguntar diretamente: "Quer que eu escreva as Fases 3/4/6 como melhor esforço não validado (claramente sinalizado no código), ou seguimos esperando o equipamento chegar?" Não presumir a resposta.
