...
uses
  ...
  Dialogs, StdCtrls, SerialNG, StrUtils, ComCtrls, CheckLst, ExtCtrls,
  ScktComp, SysUtils;
...
type
  Tfprincipal = class(TForm)
    ...
    procedure spCOMRxClusterEvent(Sender: TObject);
...
var
  ...
  procedure enviaComando(frameHex: string; tamRetorno: Integer);
...
//**Procedimento para envio de frame de comando "Nice", com cálculo de checksum, para a interface selecionada
procedure enviaComando(frameHex: string; tamRetorno: Integer);
var
        i, tamFrame: Integer;
        checksum: Byte;
...
begin
        ...
        //Gera array de bytes para envio (Serial/Socket)
        ...
        if ( fprincipal.rbSerial.Checked ) then
        begin
         // SERIAL
         fprincipal.spCOM.ClusterSize := tamRetorno;
         fprincipal.spCOM.SendData(@frameByte[0], tamFrame);
        end
        else
         if ( fprincipal.rbTCP.Checked ) then
         begin
          // TCP
...
//*****************************************
        ...
        begin
         ...
         begin
          ...
          toutComando(True, 5);
          enviaComando('0046', 42);
         end
         else
          spCOM.ClusterSize := 20;  // SERIAL: Para recebimento de evento automático (20 bytes) - Comando 4
...
//***********************************
        ...
        begin
         ...
         begin  //Leitura terminada!
          enviaComando('002B', 20);  // Cancelando timeout do Guarita (sem resposta)
...
var
        i: Integer;
begin
        //Lista 254 portas seriais
        for i:= 1 to 254 do
         cbPorta.Items.Add('COM' + IntToStr(i));
        cbPorta.ItemIndex := 0;
...
//Iniciar ou Terminar conexão Serial/TCP
        ...
        begin  //Conecta
         ...
         begin
          // Conexão SERIAL
          spCOM.CommPort := cbPorta.Text;
          spCOM.BaudRate := StrToInt(cbBaudrate.Text);

          spCOM.Active := True;

          if (spCOM.Active) then
          begin
           spCOM.ClusterSize := 20;  // SERIAL: Para recebimento de evento automático (20 bytes) - Comando 4
...
procedure Tfprincipal.spCOMRxClusterEvent(Sender: TObject);
var
        qt: Integer;
        err: Cardinal;
        pd: PByte;
...
// SERIAL: Evento de recepção
        pd := spCOM.ReadNextCluster(qt, err);

        if (err = 0) and (qt > 0) then
        begin
         recepcaoResposta(pd, qt);
        end;
end;
...
//Acionamento Relé 1 - RECEPTOR
        ...
        enviaComando('000D'+frame_hex, 20);
end;
...
// Botão "CADASTRAR" (aba "Cadastrar Dispositivo")
        ...
        begin
         ...
         toutComando(True, 5);
         enviaComando('004300'+frame_hex, 5);
        end
        else
...
procedure Tfprincipal.rbSerialClick(Sender: TObject);
begin
...
procedure Tfprincipal.rbTCPClick(Sender: TObject);
begin
// Conexão TCP marcada
...
// Botão "Apagar Dispositivo"
        ...
        toutComando(True, 5);
        enviaComando('004304'+frame_hex, 5);
end;
