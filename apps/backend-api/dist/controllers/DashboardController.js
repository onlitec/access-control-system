        ...
        this.getStats = async (req, res) => {
            try {
                ...
                const [totalResidents, totalOwners, ownersResiding, totalProviders, totalStaff] = await Promise.all([
                    index_1.prisma.person.count({ where: { orgIndexCode: { in: residentCodes }, ...residesFilter } }),
                    index_1.prisma.person.count({ where: { orgIndexCode: { in: residentCodes }, ...ownerFilter } }),
                    index_1.prisma.person.count({ where: { orgIndexCode: { in: residentCodes }, AND: [residesFilter, ownerFilter] } }),
