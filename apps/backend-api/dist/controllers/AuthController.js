...
class AuthController {
    ...
    static async refreshToken(req, res) {
        ...
        try {
            ...
            // janela deslizante: cada refresh bem-sucedido empurra a validade da
            // sessão mais 7d (config REFRESH_EXPIRES_IN) — quem usa o app pelo
            // menos 1x por semana nunca cai pro login; some só por inatividade
            // real ou revogação explícita.
            await database_1.prisma.refreshSession.update({
                where: { id: session.id },
                data: { expiresAt: (0, token_utils_1.getRefreshExpiry)() },
            });
            await AuditService_1.AuditService.logSessionAuditEvent({
                eventType: 'refresh',
                success: true,
