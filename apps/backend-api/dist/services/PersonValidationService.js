...
exports.PersonValidationService = void 0;
...
class PersonValidationService {
     ...
     */
    static async validatePerson(data) {
        const errors = [];
        ...
        if (data.unitId) {
            ...
            if (data.unit_number || data.block || data.tower) {
                console.warn(`[PersonValidation] Both unitId and string location fields provided. Using unitId as source of truth.`);
            }
        ...
        if (data.unit_number || data.block || data.tower) {
            ...
            if (matching.length === 0) {
                errors.push(`No unit found matching: Tower="${data.tower}", Block="${data.block}", Unit="${data.unit_number}"`);
            }
            else if (matching.length > 1) {
                errors.push(`Ambiguous: ${matching.length} units match Tower="${data.tower}", Block="${data.block}", Unit="${data.unit_number}"`);
            }
            else {
     ...
     */
    static async resolveUnitId(tower, block, unit_number) {
        if (!unit_number) {
            return null;
        }
     ...
     */
    static async suggestUnits(tower, block, unit_number) {
        const where = {};
        if (unit_number)
            where.number = { contains: unit_number };
        if (tower)
            where.tower = { name: { contains: tower } };
        if (block)
            where.block = { name: { contains: block } };
        const units = await prisma.unit.findMany({
            where,
     ...
     */
    static async checkDuplicate(unitId, firstName, lastName) {
