...
class HikCentralSyncQueueService {
    ...
    static async drainOnce() {
        ...
        for (const row of rows) {
            ...
            try {
                ...
                failed++;
                console.error(`[HikCentralSyncQueue] Falha ao processar ${row.entityType}/${row.entityId} (tentativa ${attempts}):`, error.message);
            }
        }
        return { processed: rows.length, succeeded, failed };
    }
    static async dispatch(row) {
     ...
     */
    static async reflectStatusOnEntity(entityType, entityId, status, error) {
        try {
            switch (entityType) {
                case 'SERVICE_PROVIDER':
    ...
    static async pushServiceProvider(row) {
        ...
        if (row.operation === 'delete') {
            if (payload.hikcentralPersonId) {
                await HikCentralService_1.HikCentralService.deletePerson(payload.hikcentralPersonId);
            }
        ...
        if (!payload.hikcentralPersonId) {
            const response = await HikCentralService_1.HikCentralService.addPerson({
                personGivenName,
                personFamilyName,
        ...
        const updateResponse = await HikCentralService_1.HikCentralService.updatePerson({
...
exports.HikCentralSyncQueueService = HikCentralSyncQueueService;
