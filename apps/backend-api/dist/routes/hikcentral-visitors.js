...
const isHikCentralConfigured = () => (process.env.PROVIDER_TYPE ?? 'local').toLowerCase() === 'hikcentral' &&
...
const serializeLocalProvider = (p) => {
    const now = new Date();
...
router.get('/visitantes', async (req, res) => {
    ...
    try {
        ...
        const allVisitors = await HikCentralService_1.HikCentralService.fetchVisitorsWithStatus(groupName);
...
router.get('/visitantes-atividade', async (req, res) => {
    if (!isHikCentralConfigured()) {
        const rows = await prisma.visitor.findMany({ orderBy: { createdAt: 'desc' }, take: 500 });
...
router.get('/visitantes-finalizados', async (req, res) => {
    if (!isHikCentralConfigured()) {
        const rows = await prisma.visitor.findMany({ orderBy: { createdAt: 'desc' }, take: 500 });
...
router.get('/prestadores', async (req, res) => {
    ...
    try {
        ...
        const allProviders = await HikCentralService_1.HikCentralService.fetchVisitorsWithStatus(groupName);
...
router.get('/prestadores-atividade', async (req, res) => {
    if (!isHikCentralConfigured()) {
...
router.get('/prestadores-finalizados', async (req, res) => {
    if (!isHikCentralConfigured()) {
        const now = new Date().toISOString().split('T')[0];
        const rows = await prisma.serviceProvider.findMany({
 ...
 */
router.get('/internal-providers', async (req, res) => {
    if (!isHikCentralConfigured()) {
    ...
    try {
        ...
        const result = await HikCentralService_1.HikCentralService.getPersonsByDepartment(prestadoresOrgCode);
...
router.get('/terminals', async (req, res) => {
    ...
    try {
        ...
        let result;
        if (type === 'acs') {
            result = await HikCentralService_1.HikCentralService.getAcsDeviceList(1, 100);
        }
        else {
            // Fallback para câmeras se solicitado vídeo
            result = await HikCentralService_1.HikCentralService.hikRequest('/artemis/api/resource/v1/cameras', {
                method: 'POST',
...
router.get('/person-properties', async (req, res) => {
    try {
        ...
        if (!isHikCentralConfigured())
            return res.json({ options: defaultOptions });
        try {
            ...
            const result = await HikCentralService_1.HikCentralService.hikRequest('/artemis/api/resource/v1/person/customData/list', {
                method: 'POST',
                body: JSON.stringify({})
            });
            ...
            if (result && result.data && Array.isArray(result.data)) {
                ...
                const torreField = customFields.find((f) => (f.name || '').toUpperCase() === 'TORRE' ||
...
router.get('/organizations', async (req, res) => {
    ...
    try {
        const result = await HikCentralService_1.HikCentralService.getOrgList(1, 500);
...
router.post('/residents/sync', async (req, res) => {
    if (!isHikCentralConfigured()) {
        return res.json({ success: true, count: 0, message: 'Sistema standalone: dados 100% locais, nada a sincronizar' });
    }
    try {
        const orgs = await HikCentralService_1.HikCentralService.getOrgList(1, 500);
        const orgList = orgs?.data?.list || [];
        let total = 0;
        for (const org of orgList) {
            const persons = await HikCentralService_1.HikCentralService.getPersonList({ orgIndexCode: org.orgIndexCode, pageNo: 1, pageSize: 1000 });
...
router.post('/remote-capture', async (req, res) => {
    try {
        ...
        try {
            const resolved = await HikCentralService_1.HikCentralService.resolveCameraIndexCodeByDeviceId(deviceIndexCode);
            if (resolved) {
        ...
        const imageBuffer = await HikCentralService_1.HikCentralService.captureCameraPicture(targetIndexCode);
        // 3. Retornar a imagem
