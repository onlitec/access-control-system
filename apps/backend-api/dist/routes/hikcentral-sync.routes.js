...
router.get('/queue', async (req, res) => {
    try {
        const { status, entityType, page = '1', limit = '50' } = req.query;
        ...
        const [data, count] = await Promise.all([
            database_1.prisma.hikCentralSyncQueue.findMany({
                where,
                orderBy: { createdAt: 'desc' },
                skip: (pageNum - 1) * limitNum,
                take: limitNum,
            }),
            database_1.prisma.hikCentralSyncQueue.count({ where }),
        ]);
        res.json({ data, count });
    }
    catch (error) {
...
router.post('/:id/retry', async (req, res) => {
    try {
        const row = await database_1.prisma.hikCentralSyncQueue.update({
