...
// ── helpers ────────────────────────────────────────────────────────────────
async function run(cmd, args, timeoutMs = 30000) {
...
async function tailscaleUp(authKey) {
    const bin = IS_WIN ? 'C:\\Program Files\\Tailscale\\tailscale.exe' : 'tailscale';
    const args = ['up', `--authkey=${authKey}`];
    if (!IS_WIN) {
        await run('sudo', ['-n', bin, ...args], 60000);
        return;
    }
 ...
 */
async function ensureTurn(iceServers) {
    try {
        const yml = await fs_1.promises.readFile(MEDIAMTX_YML, 'utf8');
        ...
        await fs_1.promises.writeFile(MEDIAMTX_YML, yml.replace(/\n?$/, '\n') + lines.join('\n') + '\n');
        if (IS_WIN) {
...
async function savedConfig() {
    const row = await database_1.prisma.integrationConfig.findUnique({ where: { providerType: 'cloud' } });
    return row?.enabled ? row.config : null;
}
// ── rotas (todas admin) ────────────────────────────────────────────────────
router.get('/status', auth_1.adminMiddleware, async (_req, res) => {
    ...
    if (ip) {
        try {
            ...
            if (r.status === 200)
                registered = { slug: r.data.slug, url: r.data.url };
        }
...
router.post('/enable', auth_1.adminMiddleware, async (req, res) => {
    const steps = [];
    try {
        ...
        if (!ip && authKey) {
            try {
                await tailscaleUp(authKey);
                ip = await tailscaleIp();
            }
            catch (err) {
                steps.push({ step: 'tailscale', ok: false, detail: err.message });
            }
        }
        if (!ip) {
            res.status(409).json({
...
router.post('/disable', auth_1.adminMiddleware, async (_req, res) => {
    try {
        ...
        if (slug) {
            try {
                ...
                console.warn('[Cloud] unregister falhou:', err.message);
            }
        }
        await database_1.prisma.integrationConfig.updateMany({ where: { providerType: 'cloud' }, data: { enabled: false } });
