...
export interface Detection {
  ...
  className: string;
  score: number;
  /** caixa em coordenadas NORMALIZADAS 0–1 do frame original. */
  x1: number; y1: number; x2: number; y2: number;
  cx: number; cy: number; // centroide normalizado
}
...
export class Detector {
  private static instance: Detector | null = null;
  ...
  static async shared(modelPath: string): Promise<Detector | null> {
    if (Detector.instance) return Detector.instance;
    try {
  ...
  async detect(
    ...
    for (let i = 0; i < na; i++) {
      ...
      raw.push({
        cls: bc, className: COCO_CLASSES[bc] || `id${bc}`, score: bs,
...
export { INPUT as DETECTOR_INPUT };
