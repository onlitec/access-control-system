...
function verifyOnboardingToken(token: string): OnboardingTokenPayload {
  const decoded = jwt.verify(token, config.JWT.SECRET) as OnboardingTokenPayload;
...
export async function createOnboardingLink(personId: string, hikPersonId?: string | null): Promise<string> {
  const onboardingToken = jwt.sign(
    { personId, hikPersonId: hikPersonId ?? null, type: 'onboarding' },
    config.JWT.SECRET,
    { expiresIn: '48h' }
  );
...
router.post('/confirm-cpf', async (req: Request, res: Response) => {
  try {
    ...
    if (!person.cpf || !cpfEquals(person.cpf, cpf)) {
...
router.post('/selfie', async (req: Request, res: Response) => {
  try {
    ...
    const result = await FaceMatchService.compareFaces(person.photoUrl, photoBase64);
...
router.post('/pending-reviews/:personId/approve', async (req: Request, res: Response) => {
  try {
...
router.post('/pending-reviews/:personId/reject', async (req: Request, res: Response) => {
  try {
    const { personId } = req.params;
