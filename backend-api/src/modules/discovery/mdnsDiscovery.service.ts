...
function parseMdnsResponse(buf: Buffer): { name: string; ips: string[] } | null {
  try {
    ...
    while (i < buf.length - 10) {
      ...
      if (type === 1 && rdLen === 4) {
        ...
        if (!ip.startsWith('169.254') && !ip.startsWith('127.')) ips.push(ip);
      }
      i += 10 + rdLen;
    }
  ...
  return new Promise((resolve) => {
    ...
    socket.on('message', (buf) => {
      ...
      for (const ip of parsed.ips) {
        ...
        const deviceType = inferDeviceType([], parsed.name, manufacturer);
