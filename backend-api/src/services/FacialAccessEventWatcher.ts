...
import { mapFacialEvent } from './FacialAccessService';
 ...
 */
export class FacialAccessEventWatcher {
  ...
  private async watchLoop(
    device: { id: string; name: string; ip: string; port: number; username: string; password: string },
    handle: { stop: boolean },
  ): Promise<void> {
    ...
    while (!handle.stop) {
      try {
        ...
        for await (const chunk of res.body) {
          ...
          if (buffer.length > 262144) buffer = buffer.slice(-65536); // proteção contra lixo sem boundary
        }
      } catch (err: any) {
        if (!handle.stop) {
  ...
  private async handleEvent(
    ...
    if (payload?.eventType !== 'AccessControllerEvent') return;
    const ev = payload.AccessControllerEvent;
    if (!ev) return;

    const mapped = mapFacialEvent(ev.majorEventType, ev.subEventType);
    if (!mapped) return;

    const doorNo: number | null = ev.doorNo ?? null;
    const door = doorNo != null
      ? await prisma.facialAccessDoor.findUnique({ where: { deviceId_doorNo: { deviceId: device.id, doorNo } } })
      : null;

    const cardNo: string | null =
      ev.employeeNoString ?? (ev.employeeNo != null ? String(ev.employeeNo) : null);
    const person = cardNo
...
export const facialAccessEventWatcher = new FacialAccessEventWatcher();
