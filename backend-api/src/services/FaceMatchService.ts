...
async function decodeImage(source: string) {
  ...
  if (source.startsWith('http://') || source.startsWith('https://') || source.startsWith('/')) {
    ...
    if (!res.ok) throw new Error(`Falha ao buscar imagem de referência (HTTP ${res.status})`);
...
export interface FaceCompareResult {
  match: boolean;
  similarity: number; // distância euclidiana (menor = mais parecido)
  noFaceDetected?: 'reference' | 'candidate';
}
...
export class FaceMatchService {
   ...
   */
  static async compareFaces(referencePhoto: string, candidatePhoto: string): Promise<FaceCompareResult> {
