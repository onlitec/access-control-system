...
export interface BackfillResult {
  matched: number;
  orphans: Array<{ personId: string; name: string; tower?: string; block?: string; unit_number?: string }>;
  ambiguous: Array<{ personId: string; tower?: string; block?: string; unit_number?: string; unitCount: number }>;
...
export class PersonMigrationService {
   ...
   */
  static async backfillUnitIds(dryRun: boolean = true): Promise<BackfillResult> {
    const result: BackfillResult = { matched: 0, orphans: [], ambiguous: [], alreadyMigrated: 0 };
    ...
    for (const p of persons) {
      ...
      if (p.unit_number) whereConditions.number = p.unit_number;
      if (p.tower) whereConditions.tower = { name: p.tower };
      if (p.block) whereConditions.block = { name: p.block };
  ...
  static async validateMigration(): Promise<ValidationResult> {
    ...
    if (stats.orphans > 0) {
      issues.push(
        `Found ${stats.orphans} orphaned persons without unitId but with location data (tower/block/unit_number)`
      );
    }
   ...
   */
  static async getOrphansReport(): Promise<Array<{ personId: string; name: string; tower?: string; block?: string; unit_number?: string; createdAt: Date }>> {
   ...
   */
  static async getAmbiguousReport(): Promise<Array<{ personId: string; name: string; tower?: string; block?: string; unit_number?: string; matchingUnits: number }>> {
    const persons = await prisma.person.findMany({
      where: { unitId: null },
      select: { id: true, firstName: true, lastName: true, tower: true, block: true, unit_number: true }
    });
    ...
    for (const p of persons) {
      if (!p.tower && !p.block && !p.unit_number) continue;
   ...
   */
  static async fixPerson(personId: string, unitId: string): Promise<void> {
    // Verify unit exists
   ...
   */
  static async batchFix(fixes: Array<{ personId: string; unitId: string }>): Promise<number> {
    let count = 0;
    for (const fix of fixes) {
      try {
        ...
        console.error(`[PersonMigration] Failed to fix ${fix.personId}: ${(err as Error).message}`);
      }
    }
    return count;
  }
