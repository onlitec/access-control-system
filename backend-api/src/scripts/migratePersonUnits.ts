...
async function main() {
  ...
  if (reportType) {
    ...
    if (reportType === 'orphans') {
      const orphans = await PersonMigrationService.getOrphansReport();
      ...
      const ambiguous = await PersonMigrationService.getAmbiguousReport();
  ...
  if (fixArg) {
    ...
    const fixed = await PersonMigrationService.batchFix(fixes);
  ...
  const result = await PersonMigrationService.backfillUnitIds(isDryRun);
  ...
  if (!isDryRun) {
    ...
    const validation = await PersonMigrationService.validateMigration();
