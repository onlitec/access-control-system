...
export function DoorbellCapture({ onCapture, className = '' }: Props) {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/doorbell/devices`, {
      ...
      .then((r) => r.json())
      .then((data) => {
        const list: DoorbellDevice[] = data.devices ?? [];
        setDevices(list);
  ...
  async function capture() {
    ...
    try {
      const res = await fetch(`${API_BASE}/doorbell/devices/${selectedId}/snapshot`, {
        headers: { Authorization: `Bearer ${getToken()}` },
      });
    ...
    <div className={`space-y-3 ${className}`}>
      ...
      <div className="flex gap-2">
        {devices.length > 1 && (
          <select
            value={selectedId}
            onChange={(e) => { setSelectedId(e.target.value); setPreview(null); setCaptured(false); }}
          ...
          >
            <option value="">Selecionar videoporteiro...</option>
            {devices.map((d) => (
        ...
        {devices.length === 1 && (
          <span className="flex-1 px-3 py-2 rounded-lg bg-zinc-900 border border-zinc-800 text-sm text-zinc-400">
            {devices[0].name}{devices[0].location ? ` — ${devices[0].location}` : ''}
          </span>
        )}
