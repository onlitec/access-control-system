...
import { getInternalProviders, InternalPerson } from '@/services/api';
...
export default function InternalProvidersPage() {
    const [persons, setPersons] = useState<InternalPerson[]>([]);
    const [filtered, setFiltered] = useState<InternalPerson[]>([]);
    ...
    const loadPersons = async () => {
        ...
        setError('');
        try {
            const result = await getInternalProviders();
            setPersons(result.data || []);
            setLastUpdate(new Date());
        } catch (err: any) {
            setError(err.message || 'Erro ao carregar prestadores internos');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="page">
            <div className="page-header">
                <div>
                    <h1><HardHat size={24} /> Prestadores Internos</h1>
                    <p>
                    ...
                    <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: 4 }}>
                        Prestadores de serviços permanentes do condomínio — cadastrados no módulo de pessoas/ACS
                    </p>
                </div>
                    ...
                    <tbody>
                        {loading ? (
                                ...
                                <tr key={p.id}>
                                    <td className="td-name">
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                            <div className="person-avatar-sm" style={{ background: avatarGradient(p.person_name || '?') }}>
                            ...
                            <tr><td colSpan={5} className="table-empty">
                                {search ? 'Nenhum prestador encontrado para a busca.' : 'Nenhum prestador cadastrado no departamento PRESTADORES.'}
                            </td></tr>
                        )}
                    </tbody>
                </table>
