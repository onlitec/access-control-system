...
import { getAllVisitors, HikCentralVisitor } from '@/services/api';
import { UserCheck, Search, Loader2, AlertTriangle, Clock, Car, RefreshCw, Phone, FileText } from 'lucide-react';
...
export default function VisitorsPage() {
    const [visitors, setVisitors] = useState<HikCentralVisitor[]>([]);
    ...
    const countByStatus = (s: number) => visitors.filter(v => v.appoint_status === s).length;
        ...
        <div className="page">
            <div className="page-header">
                <div>
                    ...
                    <p>
                        {visitors.length} visitantes cadastrados no grupo <strong>VISITANTES</strong>
                        {lastUpdate && <span className="text-muted" style={{ marginLeft: 8, fontSize: '0.8rem' }}>
                            · Atualizado às {lastUpdate.toLocaleTimeString('pt-BR')}
                        </span>}
                    </p>
                </div>
                <button className="btn btn-ghost" onClick={loadVisitors} disabled={loading}>
                ...
                <table className="data-table">
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            <tr><td colSpan={7} className="table-empty">Nenhum visitante encontrado.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
            ...
            <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: '0.5rem' }}>
                Exibindo {filtered.length} de {visitors.length} visitantes
            </p>
        </div>
    );
}
